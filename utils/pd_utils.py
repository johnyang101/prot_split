...
'''
Commented lines below are dependent on files from Jason's pd_v2 "data" package. 
We aim to use his utils relevant to checkpointing. TODO: Integrate the whole data package here.
'''
...
#         residue_constants.restypes[x] for x in aatype])
...
UNPADDED_FEATS = [
    't', 'rot_score_norm', 'trans_score_norm',
]
RIGID_FEATS = [
    'rigids_0', 'rigids_t'
]
...
def parse_pdb_feats(pdb_name: str, pdb_path: str, scale_factor=1.):
    """
    Args:
...
def compose_rotvect_pytorch(v1, v2):
    """composes rotation vectors and returns new rotation vectors
    """
    R3 = torch.einsum('Njk,Nkl->Njl',
            tgm.angle_axis_to_rotation_matrix(v1),
            tgm.angle_axis_to_rotation_matrix(v2))
    v3 = tgm.rotation_matrix_to_angle_axis(R3[:, :3])
    return v3
...
def write_checkpoint(
    ...
    write_pkl(ckpt_path, {**ckpt_dict}, use_torch=use_torch)
